...
class TinyImageNet_C(Dataset):
    def __init__(self, root, name, transform=None):
        self.root_dir = os.path.join(root, name)
        self.transform = transform
        # self.train_dir = os.path.join(self.root_dir, "1")

        self._create_class_idx_dict()
    ...
    def _create_class_idx_dict(self):

        label_dir = os.path.join(self.root_dir, "1")

        if sys.version_info >= (3, 5):
            classes = [d.name for d in os.scandir(label_dir) if d.is_dir()]
        else:
    ...
    def _make_dataset(self):
        ...
        for file_name in dirs_name:
            ...
            img_root_dir = os.path.join(self.root_dir,file_name)
